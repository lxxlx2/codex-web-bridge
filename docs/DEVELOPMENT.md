...
- `standalone-dev`: normal development and release-candidate preparation.
- `main`: release boundary.
- release tags: immutable public checkpoints.

Candidate-bound release evidence is invalidated whenever the candidate commit changes, including documentation-only commits when the release policy requires exact SHA identity.
