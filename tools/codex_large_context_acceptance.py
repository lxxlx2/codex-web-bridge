...
def build_final_prompt() -> str:
    prompt = (
        ...
        "如果你已经收到过真实 exec_command 的成功 function_call_output，"
        "就不得随后声称当前会话没有 exec_command；自动 compaction 也不改变客户端工具可用性。"
        "在 large_context/result.txt 被正确写入并重新读取确认之前，"
        "第一轮要求记住的精确合成令牌仍是当前未完成任务所必需的连续性状态，"
